...
def _simulate(spec):
    ...
    shutil.copytree(os.path.join(CWD, "BestInClass"), os.path.join(wor_dir, "BestInClass"))
    # shutil.copytree(os.path.join(CWD, "energyplus"), os.path.join(wor_dir, "energyplus"))
    # Change the working directory so that the right checkout is loaded
    os.chdir(os.path.join(wor_dir, "BestInClass"))
    ...
    s.simulate()
    s._check_simulation_errors(out_dir)

    # Copy results back
    res_des = os.path.join("/mnt/hgfs/E", "simulations", spec["name"])
...
if __name__=='__main__':
    ...
    list_of_cases = generate_cases.get_all_cases()
